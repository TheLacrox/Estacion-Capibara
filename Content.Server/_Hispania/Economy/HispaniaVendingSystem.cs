...
/// </summary>
public sealed class HispaniaVendingSystem : EntitySystem
{
    [Dependency] private readonly HispaniaBankSystem _bankSystem = default!;
    ...
    private void OnAttemptPurchase(EntityUid uid, VendingMachinePriceComponent priceComp, ref AttemptVendingPurchaseEvent args)
    {
        ...
        {
            _popup.PopupEntity(
                Loc.GetString("hispania-vending-insufficient-funds", ("price", price)),
                uid, args.Buyer);
            args.Cancelled = true;
            return;
        }
        ...
        _popup.PopupEntity(
            Loc.GetString("hispania-vending-purchased", ("price", price)),
            uid, args.Buyer);
    }
