...
public abstract partial class SharedVisualBodySystem
{
    ...
    public void CopyAppearanceFrom(Entity<BodyComponent?> source, Entity<BodyComponent?> target)
    {
        ...
        foreach (var (sourceOrgan, _) in _bodySystem.GetBodyOrgans(source, source.Comp))
        {
        ...
        [NotNullWhen(true)] out Dictionary<ProtoId<OrganCategoryPrototype>, Dictionary<HumanoidVisualLayers, List<Marking>>>? applied)
    {
        ...
        foreach (var (organ, organComp) in _bodySystem.GetBodyOrgans(ent))
        {
    ...
    {
        // TODO: Trauma marking modifier UI state is incompatible with upstream HumanoidMarkingModifierState
        // This needs a custom BUI state to work properly
    }

    private void OnSetModifiers(Entity<VisualBodyComponent> ent, ref HumanoidMarkingModifierMarkingSetMessage args)
    {
        // TODO: Trauma marking modifier message format is incompatible with upstream
        // This needs a custom BUI message to work properly
    }
    ...
    private void ApplyAppearanceTo(Entity<VisualBodyComponent?> ent, HumanoidCharacterAppearance appearance, Sex sex)
    {
        ...
        // TODO: Convert List<Marking> to organ-based marking format
        // appearance.Markings is List<Marking>, but ApplyOrganMarkingsEvent expects organ-keyed dictionary
    }
